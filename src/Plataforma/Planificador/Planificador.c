...
void* planificador(void* args) {
	...
	t_socket_client* acceptClosure(t_socket_server* server) {
		...
		planificador_t_personaje* personaje = planificador_create_add_personaje(self, client);
		...
		pthread_mutex_lock(&plataforma->logger_mutex);
		log_warning(plataforma->logger,
				"Planificador nivel %s: Personaje aceptado", nivel->nombre);
		pthread_mutex_unlock(&plataforma->logger_mutex);
...
t_planificador* planificador_create(char* config_path){
	...
	new->quantum_total = config_get_int_value(config, PLANIFICADOR_CONFIG_QUANTUM);
	new->quantum_actual = new->quantum_total;

	new->tiempo_entre_movimientos = config_get_int_value(config, PLANIFICADOR_CONFIG_TIEMPO_ESPERA);
...
void planificador_destroy(t_planificador* self){
	queue_destroy_and_destroy_elements(self->personajes_ready, (void*) planificador_personaje_destroy);
	queue_destroy_and_destroy_elements(self->personajes_blocked, (void*) planificador_personaje_destroy);
	free(self);
}
...
planificador_t_personaje* planificador_create_add_personaje(t_planificador* self, t_socket_client* socket){
	planificador_t_personaje* new_personaje = malloc(sizeof(planificador_t_personaje));
